...
- The MineClaw API URL and authentication are configured by the plugin.
- All API calls require the header: `Authorization: Bearer <api-key>` (handled by the plugin configuration).
- POST requests require the header: `Content-Type: application/json`

## Quick Start Workflow

1. **Spawn a bot:** `POST <apiUrl>/api/bots` with body `{ "username": "Builder_1" }`
2. **Observe the world:** `GET <apiUrl>/api/bots/{id}/observe`
3. **Execute tools:** `POST <apiUrl>/api/bots/{id}/execute` with body `{ "tool": "fly_to", "input": { "x": 0, "y": -56, "z": 0 } }`
4. **Build structures:** `POST <apiUrl>/api/bots/{id}/build/fill-batch` with body `{ "commands": [...] }`
